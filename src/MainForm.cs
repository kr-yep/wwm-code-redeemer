...
namespace WwmRedeemer {
 public class MainForm:Form {
  ...
  TextBox sourceLink=new TextBox{Width=670,AccessibleName="Discord 討論串連結"};
  ...
  public MainForm() {
   ...
   var layout=new TableLayoutPanel{Dock=DockStyle.Fill,RowCount=7,ColumnCount=1,Padding=new Padding(14)};
   layout.RowStyles.Add(new RowStyle(SizeType.AutoSize));layout.RowStyles.Add(new RowStyle(SizeType.AutoSize));layout.RowStyles.Add(new RowStyle(SizeType.AutoSize));layout.RowStyles.Add(new RowStyle(SizeType.AutoSize));layout.RowStyles.Add(new RowStyle(SizeType.AutoSize));layout.RowStyles.Add(new RowStyle(SizeType.Percent,70));layout.RowStyles.Add(new RowStyle(SizeType.Percent,30));Controls.Add(layout);
   ...
   var sourceRow=new FlowLayoutPanel{AutoSize=true,Dock=DockStyle.Fill};sourceRow.Controls.Add(new Label{Text="Discord 討論串連結",AutoSize=true,Padding=new Padding(0,5,0,0)});sourceRow.Controls.Add(sourceLink);sourceRow.SetFlowBreak(sourceLink,true);sourceRow.Controls.Add(new Label{Text="貼上 https://discord.com/channels/…/… 網址（不是訊息內容），按 Enter 或「掃描頻道」保存。僅存本機。",AutoSize=true});layout.Controls.Add(sourceRow);
   ...
   sourceLink.KeyDown+=(s,e)=>{if(e.KeyCode==Keys.Enter){if(store==null)LoadProfile();if(store!=null&&SaveSource())Log("來源連結已保存，僅存本機。");e.SuppressKeyPress=true;}};
  ...
  void LoadProfile(){
   if(running)return;string profile=account.Text.Trim();string draftSource=String.IsNullOrEmpty(activeAccount)?sourceLink.Text:"";
   ...
   try {
    store=new Store(Path.Combine(root,"data","profiles",profile));SourceSettings.Load(store);sourceLink.Text=String.IsNullOrEmpty(SourceSettings.Url)?draftSource:SourceSettings.Url;activeAccount=profile;account.Text=profile;Directory.CreateDirectory(Path.Combine(root,"data"));File.WriteAllText(Path.Combine(root,"data","last-profile.txt"),profile);RefreshRows();
  ...
  async Task Start(bool scanOnly,bool recentOnly){
   ...
   running=true;Desktop.Stop=false;foreach(var c in new Control[]{account,sourceLink,scan,quick,full})c.Enabled=false;stop.Enabled=true;
   ...
   finally{running=false;foreach(var c in new Control[]{account,sourceLink,scan,quick,full})c.Enabled=true;stop.Enabled=false;RefreshRows();Log("已停止操作，可使用滑鼠鍵盤。紀錄已保存在本機。");}
  }
  bool SaveSource(){
   try{SourceSettings.Save(store,sourceLink.Text);sourceLink.Text=SourceSettings.Url;return true;}
   catch(Exception ex){Log("來源連結未保存："+ex.Message);sourceLink.Focus();sourceLink.SelectAll();return false;}
  }
 }
}
